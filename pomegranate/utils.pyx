...
cpdef bdot(np.ndarray X_ndarray):
    ...
    cdef np.ndarray c_ndarray = np.zeros((d, d), dtype='float64')
...
cpdef np.ndarray _convert( data ):
...
def data2array(X, shape, dtype):
    # Concatenate samples into a single array for fast access
    if shape[0] == -1:
        d = np.prod(shape[1:])
        data = np.empty((sum(len(x) for x in X), d), dtype=dtype)
        offsets = np.empty(len(X) + 1, dtype=np.intp)

        o = 0
        offsets[0] = 0
        for i, x in enumerate(X):
            l = len(x)
            data[o:o + l, :] = np.asarray(x, dtype=dtype).reshape((-1, d))
            o += l
            offsets[i + 1] = o
        ...
        d = np.prod(shape)
        X = np.asarray(X, dtype=dtype).reshape((-1, d))
        return X, len(X), np.zeros((0,), dtype=np.intp)


def weights2array(W, n, dtype=np.float64):
